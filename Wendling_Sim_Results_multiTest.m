...
% description
% ~~~~~~~~~~~
% This script produces a contour plot of the results from the Wendling
% Model, all results are shown on a single plot
...
Sim_out(9,:,9) = [Sd Ss Sd Sd Ss Ss Sd Sd Sd]; % Results for B=40
    ...
    % A =3
        plot_length =3;
        plot_width = ceil(DA/plot_length); 
                fig = figure('name',['WMSim_Results'],...
        'units','centimeters',...
        'position',[fig_left_pos fig_bottom_pos plot_width*fig_width plot_length*fig_height],...
        'papersize',[plot_width*fig_width plot_length*fig_height],...
        'filename',fig_dirandname,...
        'PaperPositionMode','auto');
    for k = 1:DA
        A = A_values(k)*10;
    subplot(plot_length,plot_width,k),imagesc(G_values,(B_values),(Sim_out(:,:,k)));
    set(gca,'Ydir','normal')
    hold on
%     view(0,90)
    set(gca,'fontsize',tick_fontsize)
    if k ==8
    xlabel('\theta_{fi} (mV)','fontsize',label_fontsize+2)
    end
    if k == 4
    ylabel('\theta_{si} (mV)','fontsize',label_fontsize+2) 
    end
...
%     axis tight
    title(['\theta_{p}=',num2str(A_values(k))],'fontsize',label_fontsize);
    box off;
    ...
    colormap(cmap)
     ...
     k=   legend('BG','SS','SDS','SR','LVR','QS');
    b = get(k,'Position');
    set(k,'Position',[1.145*b(1) b(2) b(3)/2 1.2*plot_length*b(4)]);
    legend(k,'boxoff');
    print(gcf, '-dpdf', '-r600', ['Wendling_Sim_Results_all.pdf'])
