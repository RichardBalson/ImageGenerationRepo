function output8 =Wendling_Sim(MVI,fs)
...
MVI(4,:) = MVIT;
    ...
    MVI(4,:) = ones(1,size(MVI,2))*(min_frequency+max_frequency)/2;
end
...
SimulationSettings.fs = fs;
    ...
    SimulationSettings.number_of_sigma_input = 100; % Used to determine standard deviation of input if  1: 68.27% of realisations within physiolgical range, 2: 95.45, 3: 99.73 4: 99.994
...
sampling_frequency = fs; % Sampling frequency for solutions and measurements
...
for k = normalise
    ...
    [z(k+1,1) z(k+1,5)] = PSPkernel([z(k,1); z(k,5)],dt,MVI(1,k),a,sigmoid(zs1));
    [z(k+1,2) z(k+1,6)] = PSPkernel([z(k,2); z(k,6)],dt,MVI(1,k),a,normalised_gaussian_inputSDE(k) +C(2)*sigmoid(zs2));
    [z(k+1,3) z(k+1,7)] = PSPkernel([z(k,3); z(k,7)],dt,MVI(1,k),b,sigmoid(zs3));
    [z(k+1,4) z(k+1,8)] = PSPkernel([z(k,4); z(k,8)],dt,MVI(1,k),g,C(7)*sigmoid(zs4));
...
for j = 1:ceil(no_solutions/fs)
    if j ~= ceil(no_solutions/fs)
output8((j-1)*fs+1:j*fs) = detrend(output8((j-1)*fs+1:j*fs),'constant');
    else
  output8((j-1)*fs+1:end) = detrend(output8((j-1)*fs+1:end),'constant');   
    end
end