...
Background = [3 22]; % Matrix for normal backgroud activity with [EXC SINH FINH]
Interictal = [4 25]; % Matrix for interictal activity with [EXC SINH FINH]
Onset = [4 8]; % Matrix for onset activity with [EXC SINH FINH]
Ictal = [5 12]; % Matrix for ictal activity with [EXC SINH FINH]

for j = 1:points
    PBGND(j,1) = Background(1) + randn(1,1);PBGND(j,2) = Background(2) + randn(1,1)*5;
    PIIC(j,1) = Interictal(1) + randn(1,1);PIIC(j,2) = Interictal(2) + randn(1,1)*5;
    PONS(j,1) = Onset(1) + randn(1,1);PONS(j,2) = Onset(2) + randn(1,1)*5;
    PICT(j,1) = Ictal(1) + randn(1,1);PICT(j,2) = Ictal(2) + randn(1,1)*5;
end
...
scatter(PBGND(:,1),PBGND(:,2),'k')
hold on;
scatter(PIIC(:,1),PIIC(:,2),'x','b')
hold on;
scatter(PONS(:,1),PONS(:,2),'+','r')
hold on;
scatter(PICT(:,1),PICT(:,2),'*','g')
axis([0 20 0 30]);
