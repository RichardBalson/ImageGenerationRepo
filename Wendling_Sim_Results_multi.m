...
Sim_out(9,:,9) = [Sd Ss Sd Sd Ss Ss Sd Sd Sd]; % Results for B=40
    ...
    for k = 1:DA
        A = A_values(k)*10;
    subplot(plot_length,plot_width,k),contourf(G_values,B_values,Sim_out(:,:,k),[1,Bg:0.5:Qs]);
    hold on
%     view(0,90)
    set(gca,'fontsize',tick_fontsize)
    if k ==8
    xlabel('G (mV)','fontsize',label_fontsize+2)
    end
    if k == 4
    ylabel('B (mV)','fontsize',label_fontsize+2) 
    end
